...
def main():
    ...
    if config["optimization"]["num_epochs"] != 0:
        for cluster in cluster_names:
            ...
            study.optimize(
                lambda trial: objective(trial, model, num_epochs, cluster, cluster_names, config, device, device_data, optimize_single_cluster=True),
    ...
    # Train on all clusters one at a time
    print("Training on all clusters, one at a time.")
    for cluster in cluster_names:
        # Load model on device (multi-GPU if available)
        print(f"Loading model: {model_name} ...")
        model = getattr(unet, model_name)()
        ...
        # Empty gpu memory
        print(f"Finished training on cluster: {cluster}")
        print(f"Emptying GPU memory for cluster: {cluster}")
